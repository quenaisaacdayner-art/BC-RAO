...
- SidebarFooter:
  ...
  - User menu: DropdownMenu with avatar/name showing:
    ...
    - "Log out" option (calls `supabase.auth.signOut()` then redirects to /login)
...
7. No hydration errors in console
  ...
  <done>Dashboard shell with collapsible sidebar (icon-only mode), dark/light toggle, user menu with logout, overview page with conditional empty state, professional tone throughout.</done>
</task>
...
- AUTH-03 (frontend): Logout button in sidebar user menu
