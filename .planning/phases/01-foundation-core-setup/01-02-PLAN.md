...
must_haves:
  ...
  artifacts:
    - path: "bc-rao-frontend/app/layout.tsx"
      ...
      min_lines: 20
    - path: "bc-rao-frontend/middleware.ts"
  ...
  key_links:
    ...
    - from: "bc-rao-frontend/middleware.ts"
...
**middleware.ts:**
IMPORTANT: Next.js middleware must be at `middleware.ts` in the project root (bc-rao-frontend/middleware.ts), NOT inside app/.
...
- Use server-side Supabase client to check auth state
  ...
  <verify>
1. `npm run dev` starts, visit http://localhost:3000 — page renders
2. Check middleware.ts is at project root: `ls bc-rao-frontend/middleware.ts` (not inside app/)
