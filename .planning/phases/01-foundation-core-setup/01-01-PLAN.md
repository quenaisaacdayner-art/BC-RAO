...
Note: This migration file is meant to be run in Supabase SQL Editor. Include a comment header explaining this.
  ...
  <verify>
1. Review SQL syntax: all CREATE TABLE, CREATE INDEX, ALTER TABLE ENABLE ROW LEVEL SECURITY, and CREATE POLICY statements present.
2. Count tables: `grep -c 'CREATE TABLE' migrations/001_initial_schema.sql` should output 11
3. Count RLS policies: `grep -c 'CREATE POLICY' migrations/001_initial_schema.sql` should output 36 (9 tables * 4 policies each)
4. Verify trigger exists: `grep -q 'CREATE TRIGGER on_auth_user_created' migrations/001_initial_schema.sql && echo "OK" || echo "MISSING"`
  </verify>
