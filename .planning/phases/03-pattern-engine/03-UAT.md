...
expected: |
  If a subreddit has fewer than 10 collected posts, the profiles page shows a "Need more data" message for that subreddit instead of attempting to display an invalid profile or ISC score.
awaiting: complete
...
fix: "Added analysis_task_id passthrough in ProgressTracker, analyzing phase in collect page, ManualAnalysisTrigger component, analyze proxy route, and fixed 404 API paths on campaign detail page (258b3dc, f18b7e4, b1e36f4)."
...
result: pass (after fix)
reported: "TypeError: Cannot read properties of undefined (reading 'length') when clicking any post card"
...
result: pass (after fix)
reported: "500 Internal Server Error when adding custom pattern — backend used wrong column names for syntax_blacklist table"
fix: "Rewrote add_custom_pattern to use existing table columns only (subreddit, forbidden_pattern, failure_type, confidence, is_global). Category encoded as [CAT:xxx] prefix. No migration needed (8f9e2ee)."
...
- truth: "Adding custom forbidden pattern works without 500 error"
  status: fixed
  test: 9
  artifacts: ["8f9e2ee"]
